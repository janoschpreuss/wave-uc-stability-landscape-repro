...
def SolveProblem(ref_lvl,M_modes=2,m_sol=2):
    print("Solving for M_modes =", M_modes)
    N = Ns[ref_lvl]
    #maxh = maxhs[ref_lvl]
    delta_t = 2*tend / N
    ...
    delta_t_global = 2*tend
    t_global = -tend + delta_t_global * tref
...
for M_modes in [1,2,3]:
    ...
    for ref_lvl in range( max_ref_lvl):
        result = SolveProblem(ref_lvl,M_modes=M_modes,m_sol=m_sol)
    ...
    name_str = "Cylinder" + "-"  + "-q{0}".format(q)+"-qstar{0}".format(qstar)+"-k{0}".format(k)+"-kstar{0}".format(kstar)+"msol{0}-Mmodes{1}".format(m_sol, M_modes)+".dat"
